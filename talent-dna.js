...
function hydrateProfileFromAssessment() {
  let stored;
  try {
    stored = JSON.parse(window.localStorage.getItem("skillbridgeAssessmentResult") || "null");
  } catch (error) {
    stored = null;
  }
  if (!stored) return;
  ...
  replaceText('.hero-role', trackMap[stored.persona] || "Talent Track");
  replaceText('.hero-summary', stored.copy || "");
  replaceText('.match-card strong', stored.title || "Talent Match");
  replaceText('.match-card p', stored.personality || "");
  replaceText('.completion-head span', "92%");
  setSelectorWidth('.completion-card .progress-track span', "92%");
  replaceText('.completion-card p', "النتيجة الكاملة اتحفظت في ملفك. أضف الآن مشروعًا واحدًا أو Resume لرفع قوة البروفايل أكثر.");

  const miniHighlights = document.querySelectorAll('.mini-highlight');
  if (miniHighlights[2]) {
    const strong = miniHighlights[2].querySelector('strong');
    const span = miniHighlights[2].querySelector('span');
    if (strong) strong.textContent = stored.title ? stored.title.replace("أنت: ", "") : "Talent Match";
    if (span) span.textContent = stored.paths?.[0] ? `${stored.paths[0]} roles` : "Matched roles";
  }

  if (Array.isArray(stored.strengths) && stored.strengths.length) {
    const tags = document.querySelectorAll(".skills-cloud .skill-tag");
    stored.strengths.slice(0, tags.length).forEach((item, index) => {
      tags[index].textContent = item;
      tags[index].setAttribute("data-skill-info", "هذه من أبرز نقاط قوتك بحسب آخر Full Assessment.");
    });
  }
}
